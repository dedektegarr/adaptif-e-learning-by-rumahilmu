        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title"> Kuis Materi ({{ $data->materi->topikPembahasanKelas->kelas->nama_kelas }}), Topik
                        ({{ $data->materi->topikPembahasanKelas->nama_topik }}), Materi ({{ $data->nama_materi }})</h3>
                </div>

                <div class="box-body">
                    <div class="row">
                        <form
                            action="{{ route('kelas.topikPembahasan.materi.soalKuis.post', [$data->materi->topikPembahasanKelas->kelas->id, $data->materi->topikPembahasanKelas->id, $data->materi->id, $data->id]) }}"
                            ...
                            <div id="section-tidak" style="display: none;" class="col-md-12">
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="jenis" id="jenis" class="form-control">
                                                ...
                                                @foreach ($jenis as $item)
                                                    <option value="{{ $item['level_berfikir'] }}">
                                                        {{ $item['level_berfikir'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="bank_soal_pembahasan_id" id="bank_soal_pembahasan_id"
                                                class="form-control">
                                ...
                                <div class="form-group">
                                    ...
                                    <div style="min-height: 200px; padding: 2rem; border-radius: 1rem; background-color: #E4E4E6"
                                        id="review" disabled></div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="topik_pembahasan_id" id="topik_pembahasan_id"
                                                class="form-control">
                            ...
                            <div class="col-md-12 text-center">
                                <a href="{{ route('kelas.topikPembahasan.materi.kuis', [$data->materi->topikPembahasanKelas->kelas->id, $data->materi->topikPembahasanKelas->id, $data->materi->id, $data->id]) }}"
                                    class="btn btn-warning btn-sm btn-flat"><i class="fa fa-arrow-left"></i>&nbsp;
                                    Kembali</a>
                                <button type="reset" name="reset" class="btn btn-danger btn-sm btn-flat"><i
                                        class="fa fa-refresh"></i>&nbsp;Ulangi</button>
                                <button type="submit" class="btn btn-primary btn-sm btn-flat btnSubmit"><i
                                    ...
                                    @foreach ($data->pertanyaans as $index => $pertanyaan)
                                        <tr>
                                            ...
                                            <td class="action-buttons">
                                                {{-- <a onclick="editKuis({{ $data->materi->topikPembahasanKelas->kelas->id }}, {{ $data->materi->topikPembahasanKelas->id }}, {{ $data->id }}, {{ $pertanyaan->id }})"
                                                    class="btn btn-primary btn-sm btn-flat"><i class="fa fa-edit"></i>&nbsp;
                                                    Edit</a> --}}
                                                <form
                                                    action="{{ route('kelas.topikPembahasan.materi.soalKuis.delete', [$data->materi->topikPembahasanKelas->kelas->id, $data->materi->topikPembahasanKelas->id, $data->materi->id, $data->id, $pertanyaan->id]) }}"
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Load soal to review
            $('#bank_soal_pembahasan_id').on('change', function() {
                var soalId = $(this).val();

                $.ajax({
                    url: `/bank_soal/${soalId}`,
                    type: "GET",
                    success: function(data) {
            ...
            $('#topik_pembahasan_id').on('change', function() {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    data: {
                        topik_pembahasan_id: topik_pembahasan_id
                    },
                    success: function(data) {
                        ...
                        function convertLevel(level) {
                            switch (level) {
                                ...
                                case '2':
                                    return 'Tinggi';
                                default:
                                    return 'Tidak Diketahui'; // Jika level tidak sesuai dengan nilai yang diharapkan
                            }
                        }
            ...
            function editKuis(kelas_id, topik_pembahasan_id, materi_id, kuis_id) {
                ...
                let url = "{{ url('manajemen_kelas') }}/" + kelas_id + '/topik_pembahasan_kelas/' +
                    topik_pembahasan_id + '/manajemen_materi/' + materi_id + '/kuis/' + kuis_id + '/edit';
                $.ajax({
                    url: url,
                    type: 'GET',
                    success: function(data) {
                        ...
                        if (!data.pretestExists || data.jenis_kuis === 'pretest') {
                            jenisKuisOptions +=
                                `<option value="pretest" ${data.jenis_kuis === 'pretest' ? 'selected' : ''}>Pre Test</option>`;
                        }
                        if (!data.posttestExists || data.jenis_kuis === 'posttest') {
                            jenisKuisOptions +=
                                `<option value="posttest" ${data.jenis_kuis === 'posttest' ? 'selected' : ''}>Post Test</option>`;
                        }
